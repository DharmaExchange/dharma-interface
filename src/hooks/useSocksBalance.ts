...
const SOCKS = new Token(SupportedChainId.BSC, SOCKS_CONTROLLER_ADDRESSES[SupportedChainId.BSC], 0)

export function useHasSocks(): boolean | undefined {
  const { account, chainId } = useActiveWeb3React()

  const balance = useTokenBalance(account ?? undefined, chainId === SupportedChainId.BSC ? SOCKS : undefined)
